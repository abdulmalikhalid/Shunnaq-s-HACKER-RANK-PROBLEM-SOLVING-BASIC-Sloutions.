 ...
 */
    ...
    for (int i = 0; i < n; i++)
    {
        prefix[i + 1] = prefix[i] + numbers[i]; // nums = [10 , 5 , 2] -> p = [0 , 0 , 0 , 0] implies to p[1] = p[0] + nums[0] = 0 + 10 etc..
    }
    
    // Build prefix count of zeros
    vector<long> zeroCount(n + 1, 0);
    for (int i = 0; i < n; i++) {
    ...
    for (auto& q : queries)
    {
        ...
        long zeros = zeroCount[r] - zeroCount[l - 1]; // num of zeros
        
        // Add x for each zero occurrence
        result.push_back(rangeSum + zeros * x);
    }
